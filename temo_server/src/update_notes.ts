...
router.post('/', authenticateSupabaseToken,async (req: Request, res: Response): Promise<any> => {
  const { _userId,id, title, note_data, updatedAt } = req.body;
  ...
  if (!id || !title || (!note_data)) {
    return res.status(400).json({ error: "Missing required fields: id, title, or note_data" });
  }
  ...
  try {
    ...
    const updateResult = await client.query(
      'UPDATE notes SET title = $1, updatedat = $2, note_data = $3 WHERE id = $4 RETURNING *',
      [title, updatedAt, note_data ? JSON.stringify(note_data) : null, id]
    );
