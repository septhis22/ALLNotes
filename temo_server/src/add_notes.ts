...
router.post('/add_notes',authenticateSupabaseToken,async (req: Request, res: Response):Promise<void> => {
  const { id, title, note_data, updated_at } = req.body;
  ...
  try {
    ...
    try {
      ...
      const response = await client.query(
        'INSERT INTO notes(id, title, note_data, updatedat,owner) VALUES ($1, $2, $3, $4, $5) RETURNING id',
        [id, title, note_data ? JSON.stringify(note_data) : null, updated_at,userId]
      );
